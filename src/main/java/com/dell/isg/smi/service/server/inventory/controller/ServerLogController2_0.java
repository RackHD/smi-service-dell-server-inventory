...
import com.dell.isg.smi.service.server.inventory.mock.selog.SelLogResponse;
...
public class ServerLogController2_0 {
    ...
    @ApiOperation(value = "/get/{type}", nickname = "getLogs", notes = "This operation allow user to collect SEL or LC logs from the server using `. Type - LC or SEL.", response = Object.class, responseContainer = "List")
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Success", response = SelLogResponse.class, responseContainer = "List", reference="Please refer WSMAN Schema section for the response schema."), @ApiResponse(code = 401, message = "Unauthorized"), @ApiResponse(code = 403, message = "Forbidden"), @ApiResponse(code = 404, message = "Not Found"), @ApiResponse(code = 500, message = "Failure") })
